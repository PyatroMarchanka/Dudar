...
export interface BagpipeConfig {
  ...
  // Notes played in the upper register (overblown) on flutes. The pipe is drawn
  // with `images.mainPipeOverblown` while one of them is active.
  overblownNotes?: string[];
}
...
export interface BagpipeHolesPositions {
  ...
  // Finger centers, one per entry of fingersMaps indexes. Defaults to all lines but the last.
  fingersYPositions?: number[];
}
