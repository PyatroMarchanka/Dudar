...
const EDuda = () => {
  ...
  const [stroi, setStroi] = useState(localStorage.getItem('stroi') || 'F#4,E4,D4,C#4,B3,A3,G3,E3,E3');
  ...
  const synth = useRef(new Tone.Sampler({
    urls: {
      A2: "../samples/A2.mp3",
      "D#2": "../samples/D#2.mp3",
      A3: "../samples/A3.mp3",
  ...
  const droneSynth = useRef(new Tone.Sampler({
    urls: {
      A2: "../samples/A2.mp3",
      "D#2": "../samples/D#2.mp3",
      A3: "../samples/A3.mp3",
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      const service = await gattServer.getPrimaryService('19b10000-e8f2-537e-4f6c-d104768a1214');
  ...
  const handleCharacteristicChange = (event: any) => {
    ...
    } else if (newValue === '000000000' || (isSoundEnabled && newValue[0] === '0')) {
  ...
  const handleUpdate = async (file: any) => {
    ...
    try {
      const ota = await bleServiceFound?.current?.getCharacteristic('19b10003-e8f2-537e-4f6c-d104768a1214');
      for (let i = 0; i < Math.ceil(data.byteLength / chunkSize); i++) {
        setProgress(`Uploaded ${Math.ceil(100 * (i * chunkSize) / data.byteLength)}%`);
      ...
      <div className="content">
        <div className="card-grid">
          <div className="card">
            ...
            <button onClick={handleDisconnect} className="disconnectButton">Disconnect</button>
            <input type="file" accept=".bin" onChange={(e) => handleUpdate(e.target.files?.[0])} style={{ display: 'none' }} />
            <span>{progress}</span>
            <p>eDuda state: <strong style={{ color: bleState === 'Disconnected' ? '#d13a30' : '#24af37' }}>{bleState}</strong></p>
          </div>
        </div>
        ...
        <div className="card-grid">
          <div className="card">
            <h2>NOTE</h2>
            <p className="reading" style={{ fontSize: '32px', color: 'green', fontWeight: 'bold' }}>{currentNote}</p>
          </div>
          <div className="card">
            <h2>STROI</h2>
            <input type="text" value={stroi} onChange={(e) => setStroi(e.target.value)} style={{ width: '300px' }} />
            <p>
              <button onClick={() => setStroi('F4,E4,D4,C4,B3,A3,G3,E3,E3')}>F4,E4,D4,C4,B3,A3,G3,E3,E3</button>
            </p>
            <p>
              <button onClick={() => setStroi('F#4,E4,D4,C#4,B3,A3,G3,E3,E3')}>F#4,E4,D4,C#4,B3,A3,G3,E3,E3</button>
            </p>
          </div>
          <div className="card">
            <h2>Settings</h2>
            <label>LEVEL</label>
            <input type="text" value={level} onChange={(e) => setLevel(e.target.value as any)} style={{ width: '500px' }} />
          </div>
        </div>
      </div>
    </div>
